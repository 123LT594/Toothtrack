...
from dataset import load_mesh_and_keypoints, project_3d_to_2d, compute_reproj_err  # noqa: E402
...
def extract_keypoints_main():
    ...
    parser.add_argument("--mesh-dir", type=str, default="demo_data/tooth_train/mesh", help="mesh文件目录")
    parser.add_argument("--output", type=str, default="demo_data/tooth_train/keypoints_3d_model.json", help="输出JSON文件")
    parser.add_argument("--verify", action="store_true", help="是否验证坐标（需要提供RT矩阵和2D标注）")
    parser.add_argument("--rt-file", type=str, default="demo_data/tooth_train/annotated_poses/000000.txt", help="RT矩阵文件（用于验证）")
    parser.add_argument("--annotations", type=str, default="demo_data/tooth_train/annotations.json", help="2D标注文件（用于验证）")
    parser.add_argument("--frame-id", type=str, default="frame_0001", help="用于验证的帧ID")
    parser.add_argument("--K-file", type=str, default="demo_data/tooth_train/cam_K.txt", help="相机内参文件（用于验证）")
...
def generate_rt_main():
    ...
    parser.add_argument("--data-root", type=str, default="demo_data/tooth_train", help="数据根目录")
    parser.add_argument("--output-dir", type=str, default="demo_data/tooth_train/annotated_poses", help="RT矩阵输出目录")
...
def prepare_inference_main():
    ...
    parser.add_argument("--source-dir", type=str, default="demo_data/tooth_train", help="源数据目录（训练数据）")
    parser.add_argument("--target-dir", type=str, default="demo_data/tooth", help="目标数据目录（推理数据，新建）")
