...
try:
    ...
    import mycpp
    print(f"✅ [成功] mycpp 模块已定位: {mycpp.__file__}")
except Exception as e:
    print(f"❌ [失败] mycpp 导入崩溃！错误详情: {e}")
    # 这里如果报错，通常是因为缺 .so 依赖库
    import traceback
    traceback.print_exc()
    mycpp = None
...
class FoundationPose:
    ...
    def make_rotation_grid(self, min_n_views=40, inplane_step=60):
        # --- 强行补丁：确保 mycpp 在函数内部是可用的 ---
        global mycpp
        if mycpp is None:
            import mycpp  # 如果是 None，再次尝试本地导入
        # ------------------------------------------
    ...
    def track_one(self, rgb, depth, K, iteration, extra={}):
        ...
        print("[track_one()] pose done")
        if self.debug >= 2:
            extra["vis"] = vis
        self.pose_last = pose
        # 修改：直接返回中心化坐标系的pose，而不是原始坐标系的pose
        # 这样整个系统都使用中心化坐标系，避免频繁转换
        # 如果需要原始坐标系的pose，可以在调用处转换：pose_original = pose_centered @ get_tf_to_centered_mesh()^(-1)
