...
class YcbineoatReader:
    ...
    def get_gt_pose(self, i):
        try:
            file_path = self.gt_pose_files[i]
            if file_path.endswith('.npy'):
                pose = np.load(file_path).reshape(4, 4)
            else:
                pose = np.loadtxt(file_path).reshape(4, 4)
            return pose
        except:
    ...
    def get_mask(self, i):
        # 尝试从masks目录读取
        mask_file = self.color_files[i].replace("rgb", "masks")
        mask = cv2.imread(mask_file, -1)
        
        # 如果masks目录的文件不存在，尝试从gt_mask目录读取
        if mask is None:
            mask_file = self.color_files[i].replace("rgb", "gt_mask")
            mask = cv2.imread(mask_file, -1)
...
class BopBaseReader:
    ...
    def get_depth(self, i, filled=False):
        if filled:
            ...
            depth = cv2.imread(depth_file, -1) / 1e3  # 毫米转米
        else:
            depth_file = (
                self.color_files[i].replace("rgb", "depth").replace("gray", "depth")
            )
            # 统一使用 / 1e3 进行毫米到米的转换
            depth = cv2.imread(depth_file, -1) / 1e3 * self.bop_depth_scale
        if self.resize != 1:
            depth = cv2.resize(
                depth,
                fx=self.resize,
